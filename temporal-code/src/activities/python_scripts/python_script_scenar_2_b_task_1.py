...
from .utils import load_config, now, truncate_to_interval, push_metrics, IOBoundTaskParams
...
def task_1_s3_and_pg_cleanup(
    ...
    start_ts = now()
    scheduled_ts = truncate_to_interval(start_ts, params.scheduling_interval)

    # -------- S3 --------
    output_key = f"users-aggr-wf-{params.workflow_number}.parquet"
    ...
    cur = conn.cursor()
    cur.execute(
        f"DROP TABLE IF EXISTS user_per_days_workflow{params.workflow_number}"
    )
    conn.commit()
    cur.close()
    conn.close()
